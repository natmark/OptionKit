...
fileprivate struct OptionData : Equatable, CustomStringConvertible, CustomDebugStringConvertible {
    let option:Option
...
public struct OptionParser {
    ...
    public func parse(parameters:[String]) throws -> ParseData {
        let normalizedParams = OptionParser.normalizeParameters(parameters: parameters)
        ...
        let (parsedOptions, args) = try normalizedParams.reduce(firstCall) { tuple, next in
            ...
            if let lastOpt = optArray.last {
                ...
                if lastOpt.option.numberOfParameters > lastOpt.parameters.count {
                    ...
                    if (Option.isValidOptionString(str: next)) {
                ...
                return try self.parseNewFlag(current: tuple, flagCandidate: next)
            }
    ...
    private func parseNewFlag(current: ([OptionData], [String]), flagCandidate:String) throws -> ([OptionData], [String]) {
        ...
        if Option.isValidOptionString(str: flagCandidate) {
            for flag in self.definitions {
                if flag.matches(str: flagCandidate) {
    ...
    static func normalizeParameters(parameters:[String]) -> [String] {
        return parameters.reduce([String]()) { memo, next in
            ...
            let secondIndex = next.index(next.startIndex, offsetBy: 1)
            if next[secondIndex] == "-" {
            ...
            var params = [String]()
            for char in next[secondIndex..<next.index(next.startIndex, offsetBy: 2)].characters {
                params += ["-\(char)"]
            }
